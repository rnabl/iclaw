...
export class SecretsVault {
  ...
  async store(tenantId: string, masterKey: Buffer, input: SecretInput): Promise<void> {
    try {
      ...
      // Audit log
      auditLogger.logSecretStored(tenantId, input.provider, true);
    } catch (error) {
      auditLogger.logSecretStored(tenantId, input.provider, false, String(error));
      throw error;
    }
  }
  ...
  async retrieve(
    ...
    if (!stored) {
      auditLogger.logSecretRetrieved(tenantId, provider, forTool, false, 'Secret not found');
      return null;
    }
    ...
    if (stored.expiresAt && new Date() > stored.expiresAt) {
      ...
      auditLogger.logSecretAccessDenied(tenantId, provider, forTool, 'Secret expired');
      return null;
    }
    ...
    if (forTool && !this.isInScope(stored.scopes, forTool)) {
      ...
      auditLogger.logSecretAccessDenied(tenantId, provider, forTool, 'Tool not in scope');
      return null;
    }
    
    try {
      const decrypted = decrypt(stored.encryptedKey, stored.iv, stored.authTag, masterKey);
      auditLogger.logSecretRetrieved(tenantId, provider, forTool, true);
      return decrypted;
    } catch (error) {
      console.error(`[Vault] Decryption failed for ${tenantId}:${provider}`, error);
      auditLogger.logSecretRetrieved(tenantId, provider, forTool, false, 'Decryption failed');
      return null;
    }
  }
  ...
  async delete(tenantId: string, provider: string): Promise<void> {
    try {
      ...
      auditLogger.logSecretDeleted(tenantId, provider, true);
    } catch (error) {
      auditLogger.logSecretDeleted(tenantId, provider, false);
      throw error;
    }
  }
  ...
  async deleteAll(tenantId: string): Promise<void> {
    try {
      ...
      auditLogger.logSecretDeleted(tenantId, '*', true);
    } catch (error) {
      auditLogger.logSecretDeleted(tenantId, '*', false);
      throw error;
    }
  }
