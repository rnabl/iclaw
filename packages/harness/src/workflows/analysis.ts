...
const AnalysisInput = z.object({
  ...
  type: z.enum(['quick_intelligence', 'opportunity_analysis']).default('quick_intelligence'),
});
...
registry.register({
  ...
  description: 'Deep CMO-level analysis of a business. Provide website URL or business name from discovery results. Returns opportunity analysis, competitive intelligence, and strategic recommendations.',
  version: '1.0.0',
...
async function analysisWorkflowHandler(
  ...
  const { website, businessName, type } = params;
  
  const identifier = website || businessName || 'unknown';
  await ctx.log('info', `Starting ${type} analysis for: ${identifier}`);
  ...
  try {
    ...
    if (NABL_ANALYSIS_URL.includes('localhost') || !NABL_API_SECRET) {
      ...
      result = generateMockAnalysis(identifier, type);
    } else {
      throw error;
    }
  }
  ...
  const output: AnalysisOutput = {
    ...
    businessId: identifier,  // Use identifier (website or businessName)
    type,
